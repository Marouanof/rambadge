...
import { Search, Badge as BadgeIcon, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react';
...
const statutOptions = ['ACTIF', 'SUSPENDU', 'REVOQUE', 'EXPIRE'];

const joursRestants = (dateExpiration) => {
  if (!dateExpiration) return { text: '-', color: 'text-muted-foreground' };
  const j = Math.ceil((new Date(dateExpiration) - Date.now()) / 86400000);
  if (j < 0) return { text: 'Expiré', color: 'text-muted-foreground' };
  if (j <= 30) return { text: `${j} j`, color: 'text-[#C20831] font-semibold' };
  if (j <= 60) return { text: `${j} j`, color: 'text-[#A67C00] font-semibold' };
  return { text: `${j} j`, color: 'text-[#008B60] font-semibold' };
};

export default function BadgesDirection() {
  ...
  const [filters, setFilters] = useState({
    employe: '', statut: '', dateDebut: '', dateFin: '',
  });
  ...
  const fetchData = async (p, f) => {
    ...
    setError('');
    try {
      const params = { page: p, size: 10, sort: 'dateExpiration,asc' };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Badges de la direction</h1>
        <p className="text-sm text-muted-foreground mt-1">Liste des badges des employés de votre direction</p>
      </div>
        ...
        <CardContent className="p-6">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
              <BadgeIcon className="size-12 mb-3 text-muted-foreground/50" />
              <p className="text-sm">Aucun badge</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {data.map((b) => {
                    const jours = joursRestants(b.dateExpiration);
                    return (
                      <tr key={b.id} className="border-b last:border-0">
                        ...
                        <td className="py-3 text-sm">{b.employeMatricule || '-'}</td>
                        <td className="py-3 text-sm">{b.directionNom || '-'}</td>
                        ...
                        <td className={`py-3 text-sm ${jours.color}`}>{jours.text}</td>
                      </tr>
                    );
                  })}
                </tbody>
